...
;;      Customized by variable gnuserv-reuseframe in group w32
;;
...
;;
;; Lennart Borgman <lennart DOT borgman DOT 073 AT student DOT lu DOT se> 2006-06-19
;;      Added support for bs-buffer.

  ...
  (let ((gnuserv-frame (gnuserv-reuseframe)))
    ...
    (let ((obuf (get-file-buffer file)))
      ...
      (cond ((and window-system
		  gnuserv-frame
                  (fboundp 'frame-live-p)	;; v19 & Xemacs 19.12+
	    ...
	    ((and window-system
		  gnuserv-frame
                  (fboundp 'live-screen-p) ;; XEmacs 19.9+
...
that the buffer has been killed."
  ...
  (when (buffer-name buffer)
    ...
    ;; From http://www.emacswiki.org/cgi-bin/wiki/BufferSelection
...
called 'window'"
  ...
  :type '(choice
	  ...
	  (const :tag "Open files in new frame"
		 nil)
	  (const :tag "Open file if new frame if it not shown in any frame"
		 gnuserv-find-files-frame)
	  (const :tag "Open file if same frame if it not shown in any frame"
		 gnuserv-find-frame-or-reuse)
	  )
  :group 'w32
  )
...
Customized by `gnuserv-reuseframe'."
  (cond
   ...
   ((eq gnuserv-reuseframe 'gnuserv-find-frame-or-reuse)
    (unless (gnuserv-find-files-frame) (selected-frame)))))
